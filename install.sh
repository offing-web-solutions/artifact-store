...
#
# Notas:
#   - macOS  → binario nativo PyInstaller. Se copia a $ARTIFACT_STORE_INSTALL_DIR.
#   - Linux  → imagen Docker (`docker load`). Requiere docker en el PATH.

set -e

REPO="offing-web-solutions/artifact-store"
...
printf "  ${B_CYAN}              ╚══════╝   ╚═╝    ╚═════╝ ╚═╝  ╚═╝╚══════╝${RESET}\n"
printf "\n"
printf "  ${DIM}Self-hosted artifact storage with signed temporary URLs${RESET}\n"
printf "\n"
printf "  ${DIM}────────────────────────────────────────────${RESET}\n"
printf "\n"
...
VARIANT=""
if [ "$OS" = "linux" ]; then
  if [ -n "${ARTIFACT_STORE_VARIANT:-}" ]; then
    VARIANT="$ARTIFACT_STORE_VARIANT"
    info "Variant overridden via ARTIFACT_STORE_VARIANT=${VARIANT}"
  else
...
# Linux needs docker for the image-load flow
if [ "$OS" = "linux" ]; then
  need docker
fi
...
if [ "$OS" = "macos" ]; then
  INSTALL_DIR="${ARTIFACT_STORE_INSTALL_DIR:-$(default_install_dir)}"
...
if [ -t 0 ] || [ -c /dev/tty ]; then
  printf "  Press ${BOLD}Enter${RESET} to install or ${BOLD}Ctrl+C${RESET} to cancel... "
  read -r _ </dev/tty
fi
...
if [ "$OS" = "macos" ]; then
  ...
  # Linux: el tarball es un archivo de imagen Docker → docker load
  info "Loading Docker image..."
  _loaded=$(docker load -i "${TMP}/${PACKAGE}" | tail -n1)
  ok "${_loaded}"
fi
...
if [ "$OS" = "macos" ]; then
  ...
  printf "    ${CYAN}  --name artifact-store \\\\${RESET}\n"
  printf "    ${CYAN}  --restart unless-stopped \\\\${RESET}\n"
  printf "    ${CYAN}  -p 3008:3008 \\\\${RESET}\n"
  printf "    ${CYAN}  -v artifact-store-data:/data \\\\${RESET}\n"
  printf "    ${CYAN}  -e STORAGE_PATH=/data \\\\${RESET}\n"
  printf "    ${CYAN}  -e SIGNING_SECRET=\$(openssl rand -hex 32) \\\\${RESET}\n"
  printf "    ${CYAN}  -e PUBLIC_BASE_URL=http://localhost:3008 \\\\${RESET}\n"
  printf "    ${CYAN}  artifact-store:${VERSION}${RESET}\n"
fi

printf "\n"
printf "  ${BOLD}Open:${RESET}   ${CYAN}http://localhost:3008${RESET}\n"
printf "\n"
printf "  ${DIM}On first open you will be asked to create the admin user.${RESET}\n"
printf "  ${DIM}Sessions stay valid for 30 days.${RESET}\n"
printf "\n"
